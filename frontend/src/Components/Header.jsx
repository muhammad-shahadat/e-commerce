        ...
        <>
            ...
            <header className="fixed top-0 left-0 w-full h-20 bg-white/80 backdrop-blur-md shadow-sm z-[999] flex items-center justify-between px-6 md:px-16">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <p className="sm:block uppercase text-xl font-bold tracking-widest text-gray-800">Shopper</p>
                </div>
                ...
                <nav className="hidden md:block">
                    <ul className="flex items-center gap-10 text-gray-600 font-medium uppercase text-sm">
                        {navLinks.map((link) => (
                            <li key={link.name} onClick={() => setMenu(link.name)} className="relative group cursor-pointer">
                                <Link to={link.path}>
                                    {link.label}
                                </Link>
            ...
            >
                <div className="p-6">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button onClick={() => setIsMenuOpen(false)} className="p-2 hover:bg-red-50 cursor-pointer rounded-full text-red-400 transition-transform">
                            <X size={28} />
                        </button>
                    </div>
                    ...
                    <ul className="flex flex-col gap-1">
                        {navLinks.map((link) => (
                            <li key={link.name} onClick={() => { setMenu(link.name); setIsMenuOpen(false); }}>
                                <Link 
                                    to={link.path} 
