...
const ShopCategory = (props) => {
    ...
    // ক্যাটাগরি অনুযায়ী প্রোডাক্ট ফিল্টার করে নেওয়া (Clean approach)
    const filteredProducts = allCollection.filter(item => item.category === props.category);

    return (
        <main className={`w-full min-h-screen transition-all duration-300 ${!isMenuOpen ? "md:pl-5" : ""}`}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <div className="text-gray-500 font-medium">
                        <p className="text-sm md:text-base">
                            <span className="text-gray-900 font-bold">Showing 1-{filteredProducts.length > 12 ? 12 : filteredProducts.length}</span> out of {filteredProducts.length} products
                        </p>
                    </div>
