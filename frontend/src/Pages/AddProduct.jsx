...
import { Upload, X, Plus, Trash2, ChevronDown, Tag, Package, Loader2, Image as ImageIcon } from 'lucide-react';
...
const AddProduct = () => {
    ...
    const handleChangeCategory = (index, value) => {
        ...
        if(!value) {
            ...
            const lastSelectedId = newChain.length > 0 ? newChain[newChain.length -1] : "";
    ...
    const updateVariantOption = (vIndex, oName, value) => {
        ...
        const opt = updatedVariants[vIndex].options.find(o => o.option_name === oName);
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto bg-gray-50 min-h-screen font-sans">
            <header className="mb-8 flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">PostgreSQL & Cloudinary Architecture</p>
                </div>
                <button 
                ...
                >
                    { isPending? <Loader2 size={18} className='text-gray-300 animate-spin' /> : "Upload to Store"}
                </button>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <div className="flex items-center gap-2 mb-6 border-b pb-3 text-blue-600">
                            <Tag size={20}/> <h3 className="text-lg font-bold">Product Information</h3>
                        </div>
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Slim Fit Denim" 
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        onChange={handleChange} 
                                        className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" placeholder="0.00" 
                                    />
                                </div>
                                <div>
                                    ...
                                    <input 
                                        ...
                                        onChange={handleChange} 
                                        className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" placeholder="10"
                                     />
                                </div>
                            </div>
                            <div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <label className="flex items-center cursor-pointer gap-2 bg-gray-50 px-3 py-2 rounded-lg border">
                                <span className="text-xs font-bold text-gray-600">Enable Variants?</span>
                                <input 
                                    type="checkbox" 
                                    checked={hasVariants} 
                        ...
                        {!hasVariants ? (
                            ...
                            <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                                <div className="flex justify-end">
                                    <button onClick={addVariant} className="text-xs font-bold bg-blue-50 text-blue-600 px-4 py-2 rounded-lg hover:bg-blue-100 transition-colors">
                                        + Add New Variant
                                    </button>
                                </div>
                                {product.variants.map((v, vIndex) => (
                                    <div key={vIndex} className="p-4 border border-gray-200 bg-white rounded-xl relative shadow-sm">
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pr-10">
                                            <input value={v.options[0].option_value} onChange={(e) => updateVariantOption(vIndex, 'Color', e.target.value)} className="p-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm" placeholder="Color (Red)" />
                                            <input value={v.options[1].option_value} onChange={(e) => updateVariantOption(vIndex, 'Size', e.target.value)} className="p-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm" placeholder="Size (e.g. XL)" />
                                            <input type="number" value={v.price_modifier} onChange={(e) => handleVariantFieldChange(vIndex, 'price_modifier', e.target.value)} className="p-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm" placeholder="+ Price" />
                                            <input type="number" value={v.quantity} onChange={(e) => handleVariantFieldChange(vIndex, 'quantity', e.target.value)} className="p-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm" placeholder="Stock" /> 
                                        </div>
                                        <button onClick={() => removeVariant(vIndex)} className="absolute top-1/2 -translate-y-1/2 right-3 text-red-400 hover:text-red-600 transition-colors">
                                            <Trash2 size={20} />
                                        </button>
                                    </div>
                                ))}
                                {product.variants.length === 0 && <p className="text-center text-gray-400 text-sm py-4">Click &apos;Add New Variant&apos; to start</p>}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    {/* Media Gallery */}
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><ImageIcon size={20}/> Media Gallery</h3>
                        <div className="space-y-4">
                            <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Main Display Image</p>
                                <label className="border-2 border-dashed border-blue-100 rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 transition-all min-h-[140px]">
                                    ...
                                    {product.mainImage ? (
                                        <div className="relative w-full">
                                            <img src={URL.createObjectURL(product.mainImage)} className="h-32 w-full object-cover rounded-lg" alt="Main" />
                                            <span className="absolute top-1 left-1 bg-green-500 text-[10px] text-white px-2 py-0.5 rounded font-bold uppercase">Main</span>
                                        </div>
                                    ) : (
                                        <div className="text-center text-blue-400">
                                            <Upload className="mx-auto mb-2" />
                                            <p className="text-[10px] font-bold uppercase">Upload Cover</p>
                                        </div>
                                    )}
                                </label>
                            </div>
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Gallery Images Max-4</p>
                                <div className="grid grid-cols-3 gap-2">
                                    {product.subImages.map((img, i) => (
                                        <div key={i} className="aspect-square rounded-lg relative overflow-hidden group border border-gray-100">
                                            <img src={URL.createObjectURL(img)} className="w-full h-full object-cover" alt="Sub" />
                                            <button onClick={() => setProduct({...product, subImages: product.subImages.filter((_, idx) => idx !== i)})} className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white transition-opacity"><X size={16}/></button>
                                        </div>
                                    ))}
                                    <label className="aspect-square border-2 border-dashed border-gray-200 rounded-lg flex items-center justify-center cursor-pointer hover:border-blue-300 transition-colors">
                                        <input type="file" className="hidden" multiple onChange={handleSubImages} />
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                        ...
                        {selectedChain.map((id, index) => {
                            ...
                            return (
                                <div key={index} className="relative animate-in fade-in slide-in-from-top-1">
                                    <select 
                                        value={selectedChain[index + 1] || ""}
                                        onChange={(e) => handleChangeCategory(index + 1, e.target.value)}
